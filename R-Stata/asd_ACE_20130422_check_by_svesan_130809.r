...
### DE model:
DEmod <- mxModel( 'DE' , baseMod , MZmod , DZmod , Fullmod , MHmod , PHmod ,
  mxAlgebra( MZ.objective+DZ.objective+Full.objective+MH.objective+PH.objective , name='m2ll'),
  mxAlgebraObjective('m2ll'),
### Confidence bounds
  mxCI(c('Base.d2','Base.e2'))
)
### Fix t2-parameter at 0
DEmod@submodels$Base@matrices$t2@values <- matrix(0,1,1)
DEmod@submodels$Base@matrices$t2@free <- matrix(F,1,1)
### Fix a2-parameter at 0
DEmod@submodels$Base@matrices$a2@values <- matrix(0,1,1)
DEmod@submodels$Base@matrices$a2@free <- matrix(F,1,1)
### Fix c2-parameter at 0
DEmod@submodels$Base@matrices$c2@values <- matrix(0,1,1)
DEmod@submodels$Base@matrices$c2@free <- matrix(F,1,1)

### Fit model , running intervals makes takes much longer
DEmodFit <- mxRun( DEmod , intervals=TRUE )
summary(DEmodFit)

### Likelihood ratio test
#mxCompare(satModFit,ACmodFit)
mxCompare(TADCEmodFit,DEmodFit)
mxCompare(ADCEmodFit,DEmodFit)
mxCompare(ADEmodFit,CEmodFit)
