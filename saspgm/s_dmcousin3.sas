...
* ............: needed                                                        ;
* Note........: 130603 added full_cousins dataset where cousins have the same ;
* ............: full grand parents.                                           ;
*-----------------------------------------------------------------------------;
* Data used...: mfr2 tmpdsn.grandparents full_sibs                            ;
...
*-- First, get the grandparents to children in mfr;
...
*-- Cousins with sharing both grand parents ("full cousins") ;
proc sql;
  create table full_cousins(label='Full cousins, the same full grand parents') as
  select a.*
  from cousins as a
  left join gp1 as b
    on a.barn1=b.barn
  left join gp1 as c
    on a.barn2=c.barn
  having (b.mormor=c.mormor and b.morfar=c.morfar) or
         (b.mormor=c.farmor and b.morfar=c.farfar) or
         (b.farmor=c.mormor and b.farfar=c.morfar) or
         (b.farmor=c.farmor and b.farfar=c.farfar)
  ;
quit;
