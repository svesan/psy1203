...
* kon      1: boy, 2: girl, 3:unknown                                    ;
*                                                                        ;
* Get sex from v_individual which is scb data since more reliable than   ;
* the sex variable in the mfr (verified by Christina Norrby)             ;
*------------------------------------------------------------------------;
...
proc sql;
  ...
  select lopnrbarn as barn length=6, lopnrmor as mor length=6, lopnrfar as far length=6, bordf2,
         ...
         case kon    when 1 then '1' when 2 then '2' else '' end as sex length=1 format=$sexfmt. label='Sex',
  ...
  from connection to oracle(
     ...
     select a.lopnrbarn, a.lopnrmor, b.lopnrfar, a.dodfod, a.x_bfoddat, a.bordf2,
            c.kon, a.x_bfoddat, trim(a.x_mfoddat||'15') as x_mfoddat, a.paritet
     from v_mfr_base a
     left join v_parent b
       on a.lopnrmor=b.lopnrmor and a.lopnrbarn=b.lopnr
     left join v_individual c
       on a.lopnrbarn=c.lopnr
       where a.lopnrmor is not null and a.lopnrbarn is not null
             and a.x_bfoddat >= '19820101'
             and a.x_bfoddat < '20070101'
  );
...
*==============================================================================;
* Update the psych history data with info about source: child, mother, father  ;
*==============================================================================;
proc sql;
...
proc sql;
  ...
  left join far_first_diag as f
    on a.barn2=f.barn
  left join tmpdsn.twins as g
    on a.barn1=g.barn
  left join tmpdsn.twins as h
    on a.barn2=h.barn
  ;
  ;
quit;

data x3(label='Analysis dataset X3. No cousins')
     x3_cousins(drop=ft mor1 far1 label='Analysis dataset - cousins only')
  slask1 slask2 cslask2 slask3 cslask3;
...
%tor(data=x3,dname=psy1203B, rfile=psy1203_B.R, file=psy1203_B.csv, save=Y,
...
* ADD DIFF IN YRS BETWEEN SIBLING TO ADJUST FOR DIFFERENCES IN PREVALENCE;
data x3x;
set sasperm.x3;
agediff=year(child_bdat2)-year(child_bdat2);
run;

*-- Siblings including the full set of covariates;
%tor(data=x3x,dname=psy1203B, rfile=psy1203_Bx.R, file=psy1203_Bxx.csv, save=Y,
     class=outcome oldsib ft asd_exp bc sex1 sex2 mor_psych far_psych multiple monozyg1 monozyg2 oldpa oldma agediff,
     var=famid entry exit cens);


*-- Cousins including the full set of covariates;
*-- Note: ft not included here;
%tor(data=x3_cousins, dname=psy1203cousinB, rfile=psy1203_cousin_B.R, file=psy1203_cousin_B.csv, save=Y,
...
data _null_;
  call system('R CMD BATCH /home/svesan/ki/ABS/mep/sasproj/PSY/PSY1203/R-Stata/psy1203cousin_B.R');
run;
