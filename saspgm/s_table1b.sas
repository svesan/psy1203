...
*-----------------------;
proc sql;
  create table t0 as select outcome, ft, count(*) as pairs from x3 group by outcome,ft;
  ...
  select outcome, ft, a.barn2 as barn, year(a.child_bdat2) as byear length=4,  entry ,
         ...
         case  when hosp_dat2>0 then exit end as diag_age
...
data _null_;
  set t5;
  if _n_=1 then put 'Table 1. AD. Statistics for pairs not children';
  put _name_ ';' f ';' mh ';' ph;
run;
...
data _null_;
  set t6;
  if _n_=1 then put 'Table 1. ASD. Statistics for pairs not children';
  put _name_ ';' f ';' mh ';' ph;
run;
